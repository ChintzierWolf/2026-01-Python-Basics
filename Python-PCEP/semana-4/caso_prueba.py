"""
DEFINITION OF DONE — sistema_autenticacion.py
Variables de entrada obligatorias:
  · usuario      (str)
  · contrasena   (str)
  · ip_origen    (str — formato "x.x.x.x")
Roles y comportamiento requerido:
  superadmin:  verifica contraseña AND ip_origen.startswith("192.168.")
...
→ ¿El else final incluye el nombre del usuario en el mensaje?
...
usuario = input("Por favor introduzca el usuario: ")
contrasena = input("Por favor introduzca la contraseña: ")
ip_origen = input("Por favor introduzca la IP de origen: ")

if usuario == "superadmin":
    if contrasena == "S@perAdmin2024" and ip_origen.startswith("192.168."):
        print("\nACCESO TOTAL — superadmin: todos los módulos habilitados.")
    elif contrasena != "S@perAdmin2024":
        print("\nDENEGADO — superadmin: contraseña incorrecta.")
    elif not ip_origen.startswith("192.168."):
        print("\nDENEGADO — superadmin: acceso remoto no permitido.")

if usuario == "admin":
    if contrasena == "Admin#2024":
        print("\nACCESO |CONCEDIDO — admin: módulos de gestión habilitados.")
    elif contrasena != "Admin#2024":
        print("\nDENEGADO — admin: contraseña incorrecta.")
...
if usuario == "operador":
    if contrasena == "Oper@2024":
        print("\nACCESO CONCEDIDO — operador: módulos operativos habilitados.")
    elif contrasena != "Oper@2024":
        print("\nDENEGADO — operador: contraseña incorrecta.")

if usuario != "superadmin" and usuario != "admin" and usuario != "auditor" and usuario != "operador":
    print(f"\nDENEGADO — usuario {usuario} no encontrado en el directorio.\n")